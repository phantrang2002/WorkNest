...
import { GetAvaiEmployer } from '@/api/employerService'; // API giả định lấy dữ liệu nhà tuyển dụng
...
const EmployersPage = () => {
  ...
  useEffect(() => {
    const fetchEmployers = async () => {
      try {
        const response: any = await GetAvaiEmployer(pageNumber, pageSize); // API lấy dữ liệu nhà tuyển dụng
  ...
  const handleViewEmployer = (employerId: string) => {
    ...
    // Kiểm tra nếu userId trùng với employerId
    if (userId == employerId) {
      router.push('/my-company-profile'); // Chuyển hướng đến trang My Profile
    } else {
      router.push(`/employers/${employerId}`); // Chuyển hướng đến trang của employer
    }
  };
