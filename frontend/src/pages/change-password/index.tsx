...
import { changePassword } from '@/api/accountService'; // Giả sử bạn đã có API thay đổi mật khẩu
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (response.status === 200) {
          ...
          // Kiểm tra xem API có trả về message hay error
          const errorMessage = response.error || 'Something went wrong!';
          setError(errorMessage); // Hiển thị thông báo lỗi chi tiết từ API
          setSuccessMessage('');
        }
      } catch (error: any) {
        // Kiểm tra lỗi trong trường hợp có lỗi hệ thống hoặc API trả về lỗi không hợp lệ
        const errorMessage = error?.response?.data?.error || 'An error occurred. Please try again.';
        setError(errorMessage); // Hiển thị thông báo lỗi chi tiết
        setSuccessMessage('');
      } finally {
        setLoading(false);
      }
